def call(globalJsonSettings = '{}') {
  ...
  defaultJsonSettings = '{"useCache":"false","pushToGCP":"false","reproducibleImage":"false","dockerfileGlob":"**/*Dockerfile","dockerfileExcludes":"**/dev.Dockerfile,**/node_modules/**"}'
        ...
        try {
          sh '''
            ...
            /kaniko/executor --log-format=text \
              ...
              --cache-repo=image-nxrm.greyops.touchnet.com/cache \
              --destination=image \
              --no-push \
              --tarPath=/image.tar \
              --cleanup \
          ...
          withCredentials([file(credentialsId: 'pid-gousgnag-tnet-devops.json', variable: 'GOOGLE_APPLICATION_CREDENTIALS')]) {
            sh '''
              ...
              gcrane tag us.gcr.io/pid-gousgnag-tnet-devops/${IMAGE_NAME}:${IMAGE_TAG} ${IMAGE_TAG}-${GIT_COMMIT_HASH}
            '''
            println('\n\n\n use the following commad to pull this image. \n   docker image pull image-nxrm.greyops.touchnet.com/pid-gousgnag-tnet-devops/' + env.IMAGE_NAME + ':' + env.IMAGE_TAG + '\n\n or\n\n    docker image pull image-nxrm.greyops.touchnet.com/pid-gousgnag-tnet-devops/' + env.IMAGE_NAME + ':' + env.IMAGE_TAG + '-' + env.GIT_COMMIT_HASH + '\n\n\n')
          }
          if ( getValue(imageJsonSettings, 'pushToGCP') == 'true' ) {
            withCredentials([file(credentialsId: 'pid-gousgnab-tnet-sandbox1.json', variable: 'GOOGLE_APPLICATION_CREDENTIALS')]) {
              println('Pushing to Sandbox: pid-gousgnab-tnet-sandbox1')
              sh '''
                gcrane push /image.tar us.gcr.io/pid-gousgnab-tnet-sandbox1/${IMAGE_NAME}:${IMAGE_TAG}
                gcrane tag us.gcr.io/pid-gousgnab-tnet-sandbox1/${IMAGE_NAME}:${IMAGE_TAG} ${IMAGE_TAG}-${GIT_COMMIT_HASH}
              '''
            }
            withCredentials([file(credentialsId: 'pid-gousgnad-tnet-ucomm.json', variable: 'GOOGLE_APPLICATION_CREDENTIALS')]) {
              println('Pushing to Develop: pid-gousgnad-tnet-ucomm')
              sh '''
                gcrane push /image.tar us.gcr.io/pid-gousgnad-tnet-ucomm/${IMAGE_NAME}:${IMAGE_TAG}
                gcrane tag us.gcr.io/pid-gousgnad-tnet-ucomm/${IMAGE_NAME}:${IMAGE_TAG} ${IMAGE_TAG}-${GIT_COMMIT_HASH}
              '''
            }
            ...
            if ( isReleaseBranch(env.BRANCH_NAME) && !IMAGE_TAG.contains('SNAPSHOT') ) {
              ...
              println('Pushing to Production: pid-gousgnag-tnet-ucomm')
                sh '''
                  gcrane push /image.tar us.gcr.io/pid-gousgnag-tnet-ucomm/${IMAGE_NAME}:${IMAGE_TAG}
                  gcrane tag us.gcr.io/pid-gousgnag-tnet-ucomm/${IMAGE_NAME}:${IMAGE_TAG} ${IMAGE_TAG}-${GIT_COMMIT_HASH}
                '''
              }
              sh '''
                rm -v /image.tar

...
defaultJsonSettings = '''{
  "emailTo":"",
  "skipTests":"False",
  "jacocoExclusionPattern":"**/src/test*",
...
if (branch_name =~ /^hotfix/ && currentBuild.getNumber().toString() == "1") {
  echo "This is the first build of a new hotfix branch. No work will be done."
  currentBuild.result = 'ABORTED'
  return
}
if (branch_name =~ /^support/ && currentBuild.getNumber().toString() == "1") {
  echo "This is the first build of a new support branch. No work will be done."
  currentBuild.result = 'ABORTED'
  return
}
...
def maven_base_image_tag = ""
// Using mutable tags for maven images
if (getValue(globalJsonSettings, 'javaSourceVersion') == "1.8") {
  maven_base_image_tag = "develop-3.6.3-jdk-8-0.3.0-SNAPSHOT"
} else if (getValue(globalJsonSettings, 'javaSourceVersion') == "11") {
  maven_base_image_tag = "3.8-openjdk-11-slim"
} else if (getValue(globalJsonSettings, 'javaSourceVersion') == "11_openjdk") {
  maven_base_image_tag = "3.8-openjdk-11-slim"
} else if (getValue(globalJsonSettings, 'javaSourceVersion') == "11_eclipse_temurin") {
  maven_base_image_tag = "3.8-eclipse-temurin-11"
} else if (getValue(globalJsonSettings, 'javaSourceVersion') == "11_eclipse_temurin_alpine") {
  maven_base_image_tag = "3.8-eclipse-temurin-11-alpine"
} else if (getValue(globalJsonSettings, 'javaSourceVersion') == "17") {
  maven_base_image_tag = "3.8-openjdk-17-slim"
} else if (getValue(globalJsonSettings, 'javaSourceVersion') == "17_openjdk") {
  maven_base_image_tag = "3.8-openjdk-17-slim"
} else if (getValue(globalJsonSettings, 'javaSourceVersion') == "17_eclipse_temurin") {
  maven_base_image_tag = "3.8-eclipse-temurin-17"
} else if (getValue(globalJsonSettings, 'javaSourceVersion') == "17_eclipse_temurin_alpine") {
  maven_base_image_tag = "3.8-eclipse-temurin-17-alpine"
} else {
  echo "Unsupported javaSourceVersion; Exiting"
  return
}
def testMavenVersion = getValue(globalJsonSettings, 'forceTestMavenVer')
if (testMavenVersion.length() == 0) {
  echo "Base maven image: ${maven_base_image_tag}"
} else {
  maven_base_image_tag = "${testMavenVersion}"
  echo "Override Maven Image\nBase maven image: ${maven_base_image_tag}"
}
echo "Using base maven image: ${maven_base_image_tag}"
...
def fortify_container = ""
if (getValue(globalJsonSettings, 'enableFortifyScan').toBoolean() == true) {
  if (branch_name == "develop" ||
      branch_name =~ /^hotfix/ ||
      branch_name =~ /^support/ ||
      getValue(globalJsonSettings, 'forceFortifyScan').toBoolean() == true) {
    fortify_container = """
  - name: fortify
    image: image-nxrm.greyops.touchnet.com/pid-gousgnag-tnet-devops/image/fortify:${fortify_base_image_tag}
    ...
    echo "Fortify conditions not met; Not deploying Fortify container"
  }
} else {
  echo "Fortify disabled; Not deploying Fortify contianer"
}
...
spec:
  ...
  initContainers:
  - name: local-cacerts
    image: image-nxrm.greyops.touchnet.com/pid-gousgnag-tnet-devops/image/maven:${maven_base_image_tag}
    imagePullPolicy: IfNotPresent
    securityContext:
      runAsUser: 0
      runAsGroup: 0
  ...
  environment {
    ...
    NODE_EXTRA_CA_CERTS = "/etc/ssl/certs/TouchNetRootCA.pem"
                  ...
                  do
                    ...
                    if echo "${ARTIFACT}" | grep -Pq "\\.(war|jar)$"; then
                      ...
                      if test -f "info.json"; then
                        ...
                        curl --fail --silent --show-error --user "${userPass}" --upload-file info.json https://nxrm.greyops.touchnet.com/repository/api-portal/${JOB_NAME}/${MODULE_NAME}/
                      fi
                      ...
                      if test -f "adminreadme.txt" && test -s "adminreadme.txt"; then
                        ...
                        # if the output file gets created then upload it
                        curl --fail --silent --show-error --user "${userPass}" --upload-file adminreadme.txt https://nxrm.greyops.touchnet.com/repository/api-portal/${JOB_NAME}/${MODULE_NAME}/
                      fi
                      ...
                      if test -f "application.properties" && test -s "application.properties"; then
                        ...
                        # if the output file gets created then upload it
                        curl --fail --silent --show-error --user "${userPass}" --upload-file application.properties https://nxrm.greyops.touchnet.com/repository/api-portal/${JOB_NAME}/${MODULE_NAME}/
                      fi
                      ...
                      if test -f "bootstrap.properties" && test -s "bootstrap.properties"; then
                        ...
                        # if the output file gets created then upload it
                        curl --fail --silent --show-error --user "${userPass}" --upload-file bootstrap.properties https://nxrm.greyops.touchnet.com/repository/api-portal/${JOB_NAME}/${MODULE_NAME}/
                      fi
      ...
      steps {
        container('fortify') {
          ...
          sh '''
            ...
            curl --silent --show-error --user "${userPass}" --upload-file ./target/fortify/results.fpr https://nxrm.greyops.touchnet.com/repository/fortify/${JOB_NAME}/commits/${GIT_COMMIT}/
            curl --silent --show-error --user "${userPass}" --upload-file ./target/fortify/results.issues https://nxrm.greyops.touchnet.com/repository/fortify/${JOB_NAME}/commits/${GIT_COMMIT}/
            curl --silent --show-error --user "${userPass}" --upload-file ./target/fortify/results.pdf https://nxrm.greyops.touchnet.com/repository/fortify/${JOB_NAME}/commits/${GIT_COMMIT}/

            curl --silent --show-error --fail --user "${userPass}" --output ./target/fortify/${DOT_JOB_NAME}.fpr \
                  -X GET https://nxrm.greyops.touchnet.com/repository/fortify/${JOB_NAME}/${DOT_JOB_NAME}.fpr || echo "${DOT_JOB_NAME}.fpr does not exist. Nothing to merge."
            if [ -f "./target/fortify/${DOT_JOB_NAME}.fpr" ]; then
              ...
              # Upload fpr
              curl --silent --show-error --user "${userPass}" --upload-file ./target/fortify/merged.fpr https://nxrm.greyops.touchnet.com/repository/fortify/${JOB_NAME}/commits/${GIT_COMMIT}/
              curl --silent --show-error --user "${userPass}" --upload-file ./target/fortify/merged.fpr https://nxrm.greyops.touchnet.com/repository/fortify/${JOB_NAME}/${DOT_JOB_NAME}.fpr
              # Upload .issues
              curl --silent --show-error --user "${userPass}" --upload-file ./target/fortify/merged.issues https://nxrm.greyops.touchnet.com/repository/fortify/${JOB_NAME}/commits/${GIT_COMMIT}/
              curl --silent --show-error --user "${userPass}" --upload-file ./target/fortify/merged.issues https://nxrm.greyops.touchnet.com/repository/fortify/${JOB_NAME}/${DOT_JOB_NAME}.issues
              # Upload .pdf
              curl --silent --show-error --user "${userPass}" --upload-file ./target/fortify/merged.pdf https://nxrm.greyops.touchnet.com/repository/fortify/${JOB_NAME}/commits/${GIT_COMMIT}/
              curl --silent --show-error --user "${userPass}" --upload-file ./target/fortify/merged.pdf https://nxrm.greyops.touchnet.com/repository/fortify/${JOB_NAME}/${DOT_JOB_NAME}.pdf
            else
              curl --silent --show-error --user "${userPass}" --upload-file ./target/fortify/results.fpr https://nxrm.greyops.touchnet.com/repository/fortify/${JOB_NAME}/${DOT_JOB_NAME}.fpr
              curl --silent --show-error --user "${userPass}" --upload-file ./target/fortify/results.issues https://nxrm.greyops.touchnet.com/repository/fortify/${JOB_NAME}/${DOT_JOB_NAME}.issues
            fi
            '''
          } //withCredentials
          } //lock env.JOB_NAME+'/fortify'

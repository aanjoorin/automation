...
spec:
  initContainers:
  - name: local-cacerts
    image: image-nxrm.greyops.touchnet.com/pid-gousgnag-tnet-devops/image/node:${node_base_image_tag}
  ...
  - name: kaniko
    ...
    tty: true
  - name: nxiq
    image: image-nxrm.greyops.touchnet.com/image/nxiq-cli:1.90.0-01
    imagePullPolicy: IfNotPresent
    command:
    - cat
    tty: true
          ...
          withCredentials([usernameColonPassword(credentialsId: 'nxiq-scan',variable: 'userPass')]) {
            script {
              sh '''
               ...
               /usr/local/openjdk-8/bin/java -jar /nexus-iq-cli.jar -t ${NXIQ_STAGE} -s https://nxiq.greyops.touchnet.com/ -a ${userPass} -i ${applicationId} -r nxiq.json .
              '''
            }
          } // withCredentials
        }
        ...
        container('fortify') {
          ...
          sh '''
             ...
             if [ -f "./target/fortify/${DOT_JOB_NAME}.fpr" ]; then
               ...
               curl --silent --show-error --user "${userPass}" --upload-file ./target/fortify/results.pdf https://nxrm.greyops.touchnet.com/repository/fortify/${JOB_NAME}/${DOT_JOB_NAME}.pdf
             fi
             '''
          } //withCredentials
          } //lock env.JOB_NAME+'/fortify'

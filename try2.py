...
def create_github_repository(GITHUB_TOKEN, repo_name):
    base_url = "https://github.serge.com/api/v3/repos/la"  # Adjust the base URL if necessary
...
def cleanup_temporary_directory(directory):
    try:
        ...
        print(f"Permission error removing temporary directory '{directory}': {e}")
        print("Attempting to remove directory using subprocess...")
        try:
            subprocess.run(["rm", "-rf", directory], check=True)
            print(f"Temporary directory '{directory}' removed successfully using subprocess.")
        except Exception as e:
            print(f"Error removing temporary directory '{directory}' using subprocess: {e}")
